...
import { useGetAllPlatesQuery } from "@/lib/api/PlatesApis/allListedPlatesApi";
...
export default function AllPlates() {
  ...
  const { data, isLoading, isError } = useGetAllPlatesQuery({
    page,
    limit,
    searchTerm,
  });
  ...
  const formatPlate = (reg: string | undefined) => {
    if (!reg) return "";
