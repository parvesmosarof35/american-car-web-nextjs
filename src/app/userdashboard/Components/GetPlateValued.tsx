...
import { useCreateCheckoutForValuedMutation } from "@/lib/api/PaymentApis/getAPlateValuedApi";
import React, { useState } from "react";
import { useRouter } from "next/navigation";

export default function GetPlateValued() {
  const [createCheckoutForValued, { isLoading }] =
    useCreateCheckoutForValuedMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response?.data?.isAvailable) {
        ...
        // Create checkout
        const checkoutRes = await createCheckoutForValued(
          submissionData
        ).unwrap();
