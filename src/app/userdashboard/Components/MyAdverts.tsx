...
} from "@/lib/api/PlatesApis/myAdvartApi";
...
export default function MyAdverts() {
  ...
  const { data: statsData, isLoading: statsLoading } =
    useGetMyAdvertStatisticsQuery(selectedAdvertId!, {
  ...
  const handleStats = (advertId: string) => {
  ...
  const handleView = (advertId: string) => {
    router.push(`/plate-details/${advertId}`);
  };

  const handleEdit = async (advertId: string) => {
  ...
  const handleListNewPlate = () => {
    router.push("/userdashboard/list-plate");
  };
