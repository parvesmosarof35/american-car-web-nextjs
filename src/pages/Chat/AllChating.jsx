...
const handleReportUser = (userId, userName, userEmail,activePeer) => {
  ...
  // Header plate text: use state value only if URL channel matches the active channel, else fallback
  const headerPlateText = useMemo(() => {
    if (!activeChannel) return null;
    const fromUrlForActive = Boolean(currentChannelName) && activeChannel === currentChannelName;
    if (fromUrlForActive && navState?.registrationId) return navState.registrationId;
    const found = allChats.find((c) => c.channelName === activeChannel);
    if (!found) return null;
    return (
      found?.plateReg ||
      ...
      found?.plateNumber ||
      found?.plate?.registrationId ||
      found?.plateId?.registrationId ||
      null
    );
  }, [activeChannel, navState, allChats, currentChannelName]);
        ...
        ${isActive ? "bg-yellow-50 border-r-4 border-yellow-400" : ""}`}
                  >
                    ...
                    <div className="flex-1 min-w-0 flex items-center justify-between gap-3">
                      <h3 className="text-sm font-semibold text-gray-900 truncate hidden">
                        {chat.partnerName}
                      </h3>
                      {/* Plate badge: show reg from navigation state if this is the active channel; otherwise fallback to known plate fields */}
                      {(() => {
                        // Only show state registrationId if it came from URL and matches this chat's channel
                        const fromUrlForThisChat =
                          Boolean(currentChannelName) && chat.channelName === currentChannelName;
                        const plateText =
                          (fromUrlForThisChat && navState?.registrationId)
                            ? navState.registrationId
                            : (chat?.plateReg ||
                               chat?.registrationId ||
                               chat?.plateNumber ||
                               chat?.plate?.registrationId ||
                               chat?.plateId?.registrationId ||
                               null);
                        if (!plateText) return null;
                        return (
                          <div
                            className="hidden md:flex px-2 sm:px-3 md:px-4 lg:px-5 xl:px-6 py-1 sm:py-1.5 md:py-2 lg:py-2.5 xl:py-1 rounded-md shadow-[inset_0_-2px_2px_rgba(0,0,0,0.2)] items-center justify-center flex-shrink-0 w-[140px] h-[48px] sm:w-auto sm:h-auto sm:min-w-[130px] md:min-w-[150px] lg:min-w-[170px] xl:min-w-[180px]"
                            style={{
                              background:
                                "linear-gradient(180deg, #FAD95C 0%, #F9CD27 90.41%, #F9CD27 93.8%, #F9CD26 93.9%, #C09E1D 100%)",
                            }}
                          >
                            <span className="text-black font-mycustom whitespace-nowrap overflow-hidden text-ellipsis text-[30px] leading-[1] tracking-[0] font-medium sm:text-[40px] sm:font-medium block text-center">
                              {plateText}
                            </span>
                          </div>
                        );
            ...
            <div className="px-6 py-4 bg-white border-b border-gray-200 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {headerPlateText && (
                    <div
                      className="hidden md:flex px-2 sm:px-3 md:px-4 lg:px-5 xl:px-6 py-1 sm:py-1.5 md:py-2 lg:py-2.5 xl:py-1 rounded-md shadow-[inset_0_-2px_2px_rgba(0,0,0,0.2)] items-center justify-center flex-shrink-0 w-[150px] h-[56px] sm:w-auto sm:h-auto sm:min-w-[140px] md:min-w-[160px] lg:min-w-[180px] xl:min-w-[200px]"
                      style={{
                        background:
                          "linear-gradient(180deg, #FAD95C 0%, #F9CD27 90.41%, #F9CD27 93.8%, #F9CD26 93.9%, #C09E1D 100%)",
                      }}
