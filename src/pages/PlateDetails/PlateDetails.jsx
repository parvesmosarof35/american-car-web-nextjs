...
export default function PlateDetails() {
  ...
  const handleSubmitEnquiry = (e) => {
    ...
    navigate(`/login?email=${emailQP}&redirect=${redirect}`);
  };
  ...
  useEffect(() => {
    try {
      ...
      navigate(
        ...
        { state: { enquiryMessage: draft.message, enquiryEmail: draft.email } }
      );
    } catch {
      // ignore parse errors
    }
      ...
      <div>
        ...
        {similarPlates.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-gray-300 p-8 text-center bg-white">
            ...
            <p className="text-gray-700 mb-4">No similar plates are available right now.</p>
            <button
            ...
            {similarPlates.map((item, index) => (
              ...
              >
                ...
                <div className="text-xl">
                  {item.status === "sold" ? (
                    <>
                      <span className="inline-block px-2 py-0.5 rounded bg-red-100 text-red-700 text-sm mr-2">Sold</span>
                      {item.price && (
                    ...
                    <>
                      <span className="inline-block px-2 py-0.5 rounded bg-green-100 text-green-700 text-sm mr-2 capitalize">{item.status}</span>
